...
@onready var label_battery = $EnergyLabel
@onready var label_battery_counter = $TextureRect/BatteryCounter
@onready var battery_initial_size = battery_bar.size.x
@onready var health_initial_size = health_bar.size.x


func _process(_delta):
	label_battery_counter.text = str(player.consumables["battery"]["amount"])
